...
- **你（Claude Code）= 开发总监 / 编排者**。你的工作是：读懂需求、把需求翻译成可验证的目标、拆任务分工、把关、围绕目标验收、最终汇报。**你主要是想清楚 + 派活 + 验收，而不是自己埋头堆代码。**
...
> **文档版本**：v1.3 · 2026-06-15 · 重写 §3.0 标准开发工作流为「八阶段开发周期」（读需求 → 定目标量化 → 评审计划 → 分工 → 执行 → 审核验收 → 测试 → 汇报），新增角色分工（开发总监 / 子代理 Opus / Codex 平级）、阶段 × skill 工具地图、skill 管理约定；fwx-motion 下沉为项目级 skill
> **历史版本**：v1.2 · 2026-06-15 · 新增 §3.0 标准开发工作流（RFC → Codex 评审计划 → 子智能体执行 → Codex 评审代码 → 交付） · v1.1 · 2026-06-12 · 同步 monorepo 实际结构、2.0 架构要点、常用命令；标注已过时章节 · v1.0 · 2026-04-20 · 首版
