  ...
  try {
    ...
    if (coverPostId !== undefined) {
      if (coverPostId === null || coverPostId === '') {
        ...
        if (!mongoose.Types.ObjectId.isValid(coverPostId)) {
          return res.status(400).json({ error: '封面作品无效' })
        }
        const belongsToCollection = await CollectionItem.exists({
          collectionId: collection._id,
          postId: coverPostId,
        })
        if (!belongsToCollection) {
          return res.status(400).json({ error: '封面作品必须已在该合集内' })
        }
...
router.delete('/:id/items/:postId', authenticate, async (req, res) => {
  try {
    ...
    if (collection.coverPostId && String(collection.coverPostId) === String(postId)) {
      collection.coverPostId = undefined
      await collection.save()
    }
    res.json({ success: true })
