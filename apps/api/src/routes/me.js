...
// 作品库合一（RFC-024）：「我的作品数」以 DroneDesign 为唯一真相源，不再数旧 Project。
 ...
 * projectCount 实时查 DroneDesign（作品库唯一真相源）；时长由埋点累计；课时数取 completedLessons 长度。
 */
router.get('/stats', async (req, res) => {
  try {
    const [user, projectCount] = await Promise.all([
      ...
      DroneDesign.countDocuments({ ownerId: req.userId }),
    ])
