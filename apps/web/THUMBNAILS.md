...
> 更新时间：2026-08-15
>
> 适用范围：从官方 GLB 展示资产生成零件缩略图
>
> 替代关系：只说明开发期缩略图流程；零件元数据以 `@fwx/parts-schema` registry 为准
...
在浏览器中访问：
```
http://localhost:5173/tools/thumbnail-generator.html
```

### 3. 生成所有缩略图

1. 点击 **"生成所有缩略图"** 按钮
2. 等待全部 94 个模型渲染完成（约 1-2 分钟）
3. 页面会显示所有生成的缩略图预览
...
public/
  ...
  thumbnails/       # 预渲染缩略图 (.png)
    ...
    ...
tools/
  thumbnail-generator.html  # 仅开发期使用的缩略图生成工具，不进入生产构建
```
...
如需修改缩略图样式，编辑 `tools/thumbnail-generator.html`：
...
- `tools/thumbnail-generator.html` - 仅开发期使用的浏览器端生成器
