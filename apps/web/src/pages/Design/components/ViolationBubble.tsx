...
export function ViolationBubble({ violation }: ViolationBubbleProps) {
  const [dismissedViolation, setDismissedViolation] = useState<Violation | null>(null)
  const [eventViolation, setEventViolation] = useState<Violation | null>(null)
  const eventTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)

  // From props (click path)
  useEffect(() => {
    if (!violation) return
    const timer = setTimeout(() => setDismissedViolation(violation), 2500)
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (v) {
        ...
        setEventViolation(v)
        eventTimerRef.current = setTimeout(() => setEventViolation(null), 2500)
      }
    }
    window.addEventListener('fwx-violation', handler)
    return () => {
      window.removeEventListener('fwx-violation', handler)
      if (eventTimerRef.current) clearTimeout(eventTimerRef.current)
    }
  }, [])

  const current = eventViolation ?? (violation !== dismissedViolation ? violation : null)
