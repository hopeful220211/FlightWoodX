...
/**
 * 完整排行榜独立页（RFC-018 / RFC-020 视觉整改）。真实 useLeaderboard，
 * 前三名领奖台质感高亮，总分用 BigStat 大字；每行展开四维分数
 * （设计 / 编程 / 创意 / 任务完成），保留分页与三态。
 */

/**
 * 前三名质感高亮映射，index 0/1/2 对应名次 1/2/3。
 * 金银铜以描边 ring + 小面积渐变图标底表达，避免大块色块。
 */
const PODIUM = [
  {
    icon: Crown,
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        ...
        <span className="font-grotesk text-body font-medium tabular-nums text-ink-600">{value}</span>
      </div>
      <div className="h-1.5 overflow-hidden rounded-full bg-surface-ice">
  ...
  return (
    <PageContainer className="space-y-8 py-10">
      ...
      <div className="max-w-2xl">
        <SectionLabel className="text-accent-spark">Final Results</SectionLabel>
        <h1 className="mt-3 font-grotesk text-h2 font-semibold tracking-tight text-ink-900">
          完整排行榜
        </h1>
      ...
      {isLoading && (
        <div className="space-y-4">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="h-28 animate-pulse rounded-card bg-surface-ice" />
          ))}
        </div>
      )}
      ...
      {isError && (
        <Card hoverable={false} className="py-12 text-center">
          ...
          <p className="text-body text-ink-600">{(error as Error)?.message || '加载排行榜失败'}</p>
        </Card>
      )}

      {!isLoading && !isError && items.length === 0 && (
        <Card hoverable={false} className="py-14 text-center">
        ...
        <>
          <ul className="space-y-4">
            {items.map((row, idx) => {
              ...
              return (
                ...
                >
                  <Card hoverable={false} className={podium ? podium.ring : undefined}>
                    <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
                      {/* 名次 + 选手 */}
                      <div className="flex items-center gap-4 sm:w-64">
                        <div
                          className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full font-grotesk text-title-sm font-semibold tabular-nums ${
                            podium ? podium.iconWrap : 'bg-surface-ice text-ink-400'
                          }`}
                        >
                          {RankIcon ? <RankIcon size={22} /> : row.rank}
                        </div>
                        <div className="min-w-0">
                          <div
                            className="truncate text-title-sm font-semibold text-ink-900"
                            title={row.userName}
                          >
                            {row.userName}
                          </div>
                          <div className="truncate text-body text-ink-400" title={row.projectName}>
                            {row.projectName}
                          </div>
                        </div>
                      </div>
                      ...
                      {/* 总分 */}
                      <div className="flex justify-start sm:w-28 sm:justify-end">
                        <BigStat value={row.total} label="总分" className="sm:items-end" />
                      </div>
                    </div>
                  </Card>
                </motion.li>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-3 pt-2">
              <button
                type="button"
                className="rounded-tag border border-sky-200 px-4 py-2 text-body text-ink-600 transition hover:border-accent-spark hover:text-accent-spark disabled:opacity-50 disabled:hover:border-sky-200 disabled:hover:text-ink-600"
                disabled={page <= 1}
                onClick={() => setPage((p) => Math.max(1, p - 1))}
              >
                上一页
              </button>
              <span className="font-grotesk text-body tabular-nums text-ink-500">
                {page} / {totalPages}
              </span>
              <button
                type="button"
