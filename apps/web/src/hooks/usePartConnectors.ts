...
// 同一零件在点击、拖拽和 R3F 渲染之间可能同时请求；复用进行中的提取任务。
const connectorLoadCache = new Map<string, Promise<void>>()
...
export function usePartConnectors(modelUrl: string) {
  // 当前零件资产均未使用 KHR_draco_mesh_compression，明确关闭 drei 的远程 Draco 解码器。
  const gltfResult = useGLTF(modelUrl, false)
 ...
 * @param modelUrl - 模型文件路径
 */
export const prefetchAndExtractConnectors = (modelUrl: string): Promise<void> => {
  if (connectorCache.has(modelUrl)) {
    return Promise.resolve()
  }

  const existingLoad = connectorLoadCache.get(modelUrl)
  ...
  const load = new GLTFLoader()
    ...
    .catch((error: unknown) => {
      console.error(`[PartConnectors] Failed to load ${modelUrl}:`, error)
    })
    .finally(() => {
      connectorLoadCache.delete(modelUrl)
    })

  connectorLoadCache.set(modelUrl, load)
  return load
}
